...
Abstract base type for all StaticKV structs created by the `@kvstore` macro.

This provides a common interface for all key-value store structs, enabling:
- Generic programming with StaticKV types
- Type constraints in function signatures
- Method dispatch on the abstract type
- Interface documentation and contracts

All structs generated by `@kvstore` automatically inherit from this type.

# Example
```julia
# Function that works with any StaticKV struct
function process_kvstore(kv::AbstractStaticKV)
    for key in keynames(kv)
        if isset(kv, key)
            println("\$key: \$(kv[key])")
        end
    end
...
process_kvstore(person)  # Works!
process_kvstore(product) # Works!
```
"""
abstract type AbstractStaticKV end
...
macro kvstore(struct_name, args...)
    ...
    struct_def = quote
        mutable struct $(struct_name){C <: Clocks.AbstractClock} <: StaticKV.AbstractStaticKV
            $(struct_fields...)
